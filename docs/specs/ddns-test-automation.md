...
フェーズ 5（冗長化確認: TC-15〜TC-17）は Primary VM の意図的停止が必要なため、自動化対象外とし手動試験として実施する。
...
---
- name: "Phase 1: VM / OS 確認"
  hosts: dns_servers
  become: true
  roles:
    - role: ddns-verify
      vars:
        verify_phase: phase1_os

- name: "Phase 2: BIND9 DNS 動作確認"
  hosts: hip1tk-pvdesk01    # 試験端末から DNS へ問い合わせる
  roles:
    - role: ddns-verify
      vars:
        verify_phase: phase2_dns

- name: "Phase 3: Kea DHCP 動作確認"
  hosts: hip1tk-pvdesk01
  roles:
    - role: ddns-verify
      vars:
        verify_phase: phase3_dhcp

- name: "Phase 4: DDNS 動作確認"
  hosts: hip1tk-pvdesk01
  roles:
    - role: ddns-verify
      vars:
        verify_phase: phase4_ddns
```
