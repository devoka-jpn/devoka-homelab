...
| 外形監視の実施場所 | hip1tk-pvdesk01（LAN 内クライアント代表として DNS クエリを実行） |
...
| hip1tk-pvdesk01 | Zabbix Agent 2 | 外形監視（DNS クエリ・応答確認） |
...
### 3.3 DNS パフォーマンス監視（外形監視エージェント: pvdesk01）

LAN 内クライアントの視点で実際に DNS クエリを発行し、応答時間を計測する。
Zabbix Agent 2 の `UserParameter` として以下を登録する。

```ini
# /etc/zabbix/zabbix_agent2.d/ddns_check.conf（hip1tk-pvdesk01 に配置）
...
| **外形監視失敗（Primary）** | `dns.resolve_ok.primary` = 0、継続 2 分 | pvdesk01 |
| **外形監視失敗（Secondary）** | `dns.resolve_ok.secondary` = 0、継続 2 分 | pvdesk01 |
...
| トリガー名 | 条件 | 対象 |
| :--- | :--- | :--- |
| **DNS 応答遅延（高）** | `dns.response_time.primary` > 1000ms、継続 3 分 | pvdesk01 |
| **DNS 応答遅延（高）** | `dns.response_time.secondary` > 1000ms、継続 3 分 | pvdesk01 |
...
| トリガー名 | 条件 | 対象 |
| :--- | :--- | :--- |
| **DNS 応答遅延（警告）** | `dns.response_time.primary` > 500ms、継続 5 分 | pvdesk01 |
| **DNS 応答遅延（警告）** | `dns.response_time.secondary` > 500ms、継続 5 分 | pvdesk01 |
...
hip1tk-pvdesk01 に適用するテンプレート（外形監視専用）。

```
Template: DDNS External Monitor
├── Items
│   ├── dns.response_time.primary（UserParameter）
│   ├── dns.response_time.secondary（UserParameter）
│   ├── dns.resolve_ok.primary（UserParameter）
│   └── dns.resolve_ok.secondary（UserParameter）
├── Triggers
...
| Zabbix から各 VM への疎通 | Zabbix Server → pvdns01/02/pvdesk01 の 10050/TCP が通ること |
